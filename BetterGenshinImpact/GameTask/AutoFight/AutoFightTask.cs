    ...
    public async Task Start(CancellationToken ct)
    {
        ...
        async Task<bool> CheckFightFinishAfterSwitch()
        {
            return  fightEndFlag = fightEndFlag ||
                                  (++times >= 2 && _taskParam.CheckAfterSwitch && _taskParam.FightFinishDetectEnabled
                                   && await CheckFightFinish(0, detectDelayTime));
        }
        void OcrEliteFull()
        {
            if(!_taskParam.CheckElitePickUp)
                return;
            ...
            try
            {
                ...
                while (!cts2.Token.IsCancellationRequested)
                {
                    ...
                    for (var i = 0; i < combatCommands.Count; i++)
                    {
                        ...
                        async Task AddCommand(ImageRegion imageRegion, int index,Avatar avatar, bool isE)
                        {
                            if (index <= combatScenes.AvatarCount)
                                        ...
                                        : await AutoFightSkill.IsAvatarQSkillAsync(imageRegion, index, false))
                                {
                                    ...
                                    if (isE && avatar.Name == "枫原万叶")
                                    {
                                        combatCommands.Add(new CombatCommand(avatar.Name, "attack(0.4)"));
                                        combatCommands.Add(new CombatCommand(avatar.Name, "wait(0.4)"));
                                    }

                                    if (isE && combatScenes.SelectAvatar(index).Name == "白术")
                                    {
                                        combatCommands.Add(new CombatCommand(avatar.Name, "wait(0.4)"));
                        ...
                        if (_taskParam.AutoSkill && i == combatCommands.Count - 1)
                        {
                            var imageRegion = CaptureToRectArea();
        ...
        if (_taskParam.KazuhaPickupEnabled)
        {
            ...
            if (picker != null)
            {
                if (_taskParam.CheckElitePickUp && !containElite)
                {
